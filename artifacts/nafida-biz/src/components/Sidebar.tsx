          ...
          >
            <div className="flex items-center gap-2 min-w-0">
              ...
              <span className="text-sm font-semibold truncate text-sidebar-foreground">
                {selectedCompany?.company_name || "لم يتم اختيار شركة"}
              </span>
            </div>
          ...
          {companyDropdownOpen && (
            <div className="absolute top-full right-4 left-4 mt-1 bg-white rounded-xl shadow-2xl border border-slate-100 z-50 overflow-hidden">
              {companies.length > 0 ? (
                ...
                <div className="px-4 py-3 text-sm text-slate-500 text-center">لا توجد شركات مسجلة</div>
              )}
              <div className="border-t border-slate-100">
                <button
                  onClick={() => { setCompanyDropdownOpen(false); setAddCompanyModalOpen(true); }}
                  className="w-full flex items-center gap-2 px-4 py-3 hover:bg-orange-50 text-orange-600 font-medium transition-colors"
